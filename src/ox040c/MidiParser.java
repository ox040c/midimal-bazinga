...
public class MidiParser {
	...
	/**
	 * Constructor, with one arg to take filename Open a midi file, and try to
	 * get sequence out of it.
	 *
	 * @param filename
	...
	/**
	 * private method for parsing file into an EventList calling this method
	 * would decode out necessary information move costly function out of
	 * constructor
	 * <p/>
	 * for the sake of simplicity, all events will be put into a single list,
	 * sorted by ticks
	 */
	public void init() {
		...
		case 0xf:
			if (b[0] == (byte) 0xff) {
				...
				case (byte) 0x51:
					...
					microSecondPerQuarterNote = b[3] * 256 * 256 + b[4] * 256
							+ b[5];
//					System.out.println("tick: " + event.getTick() + " msg: "
//							+ "microSecondPerQuarterNote is "
//							+ microSecondPerQuarterNote);
					break;
				case (byte) 0x58:
	...
	/**
	 * convert time(second) to ticks currently this function takes the first
	 * "set tempo" event as the conversion parameter, and later "set tempo"
	 * events would be ignored TODO: more accurate timing
	 *
	 * @param timestamp_in
	 * @return
	 */
...
class MyMidiEvent implements Comparator<MyMidiEvent> {
	 ...
	 *
	 * @param event
	 *            the MIDI event contained in the class
	 */
